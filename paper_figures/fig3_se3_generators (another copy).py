...
def panel_basis_test(fig, subspec):
    ...
    for ax, M in ((ax_l, A_local), (ax_r, A_rot)):
        ax.imshow(np.abs(M), aspect="equal", cmap=SEQ_BLUE, vmin=0.0, vmax=1.2)
    ...
    ax_l.set_title("(c)  Task-local basis structure", loc="left",
                   fontsize=7.9, fontweight="semibold", pad=4)
...
def main():
    ...
    panel_control(ax_b1,
                  ...
                  show_y=True)
    ax_b1.set_title("(b)  Paired controls", loc="left", fontsize=7.9,
