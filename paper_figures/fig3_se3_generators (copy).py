...
def panel_control(ax, series, phase_labels, progress, anchors, show_y):
    ...
    for center, name in zip(centers, phase_labels):
        ax.text(center, 0.99, name, ha="center", va="top", transform=trans,
...
def main():
    ...
    fig = plt.figure(figsize=(7.15, 4.00))
    gs = GridSpec(2, 1, figure=fig, height_ratios=[0.52, 0.58],
                  hspace=0.40, left=0.07, right=0.97, top=0.95, bottom=0.11)
    ...
    gs_b = GridSpecFromSubplotSpec(1, 3, subplot_spec=gs[1, 0],
                                   width_ratios=[1.0, 1.0, 1.02], wspace=0.24)
    ax_b = fig.add_subplot(gs_b[0, 0])
    ax_c = fig.add_subplot(gs_b[0, 1])
    ax_d = fig.add_subplot(gs_b[0, 2])
    # Flatten the three evidence panels so the bottom row reads as compact
    # supporting strips rather than full-height subfigures (the curve panels b/c
    # are flatter; the 6x4 heatmap d keeps a touch more height for legibility).
    ax_b.set_box_aspect(0.80)
    ax_c.set_box_aspect(0.80)
    ax_d.set_box_aspect(0.88)
    ...
    panel_control(ax_b,
                  ...
                  show_y=True)
    ax_b.set_title("(b)  Symmetry control", loc="left", fontsize=7.9,
    ...
    panel_control(ax_c,
                  ...
                  show_y=False)
    ax_c.set_title("(c)  Relation-constraint control", loc="left", fontsize=7.9,
                   fontweight="semibold", pad=4)

    panel_multigen_heatmap(ax_d, multi)
    ax_d.set_title("(d)  Multi-generator selectivity", loc="left", fontsize=7.9,
                   fontweight="semibold", pad=4)
