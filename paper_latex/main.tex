...
transformation law is identified through isolated generator interventions, while
...
rotation-of-the-world controls, and task-space symmetry interventions support
representing task relevance at the generator level rather than enforcing a
single relevance value for an entire reference frame.
\end{abstract}
...
object, although these generators need not share the same task meaning or the
same phase dependence. Consider keyed-to-circular insertion. Lateral socket
displacement should remain relevant throughout the skill, but axial rotation is
task-relevant only while the key constrains the gate. After key clearance, the
circular shaft enters a bore for which axial spin is a gauge degree of freedom.
The desired relation is therefore not a scalar frame weight $w_r(s)$, but a
generator-level law such as
\[
    \alpha_x(s)\approx \alpha_y(s)\approx 1,\qquad
    \alpha_\psi(s): 1 \rightarrow 0 .
\]
A single scalar frame weight faces an intrinsic conflict. Suppressing the frame
to ignore the irrelevant post-clear yaw also suppresses the still-relevant
translations; following the frame to preserve translations incorrectly
propagates yaw after the task has become axisymmetric.
...
\subsection{Core Idea: Generator-Level Task Relevance}
\label{sec:core_idea}
...
\subsection{Why Frame-Level Relevance Is Insufficient}
...
\subsection{Generator Relevance Model}
...
\subsection{Phase-Dependent Learning Model}
...
The final intervention removes the key and keyway, leaving a circular
...
\begin{figure*}[t]
    \centering
    \includegraphics[width=\textwidth]{experiment_overview_scenes.png}
...
\subsection{Generator Relevance Recovers the Phase-Dependent Switch}
...
\subsection{Generator-Level Relevance Improves Few-Shot Transfer}
...
parameterization. The smooth finite $P_{\rm diag}$ model is therefore not
claimed as the lowest-error trajectory predictor. Its role is to recover a
task-consistent generator law with a constrained, interpretable finite-action
operator.

\begin{table*}[t]
\centering
\caption{Held-out baseline benchmark on the frozen single-seed dataset.
Trajectory and endpoint errors are mean mm-equivalent values. $E_{\rm gen}$
measures isolated generator-law recovery and is not quotient-projected.}
\label{tab:baseline}
\small
\setlength{\tabcolsep}{4.2pt}
\begin{tabular}{lcccccc}
\toprule
Model & Task traj. & Endpoint & $E_{\rm gen}$ & Final trans. & Pre-yaw & Final yaw\\
\midrule
...
$N\geq5$. The most sample-starved $N=3$ comparison is detailed in
Table~\ref{tab:fewshot_n3}. The matched TP-GMM few-shot experiment is summarized in
Table~\ref{tab:fewshot_tpgmm}; $P_{\rm diag}$ wins every matched seed/subset
cell at each tested $N$. The performance gap narrows as $N$ increases and
flexible operators approach the same response, which is consistent with the
claim that the diagonal generator prior mainly improves sample efficiency
rather than replacing data.
At $N=3$, paired task-error differences (competitor minus $P_{\rm diag}$) are
$+6.23$\,mm for the full operator and $+20.86$\,mm for Generic RBF under random
subsets, and $+5.81$\,mm and $+25.14$\,mm under qualified subsets; the
$P_{\rm diag}$ win fraction is 50/50 in all four comparisons.

\begin{table*}[t]
\centering
\caption{$N=3$ few-shot comparison over five seeds and ten subsets per
protocol. Continuous entries are mean over seed means $\pm$ between-seed
standard deviation. Switch and law are fractions over 50 seed/subset cells.}
\label{tab:fewshot_n3}
\scriptsize
\setlength{\tabcolsep}{3.0pt}
\begin{tabular}{llccccccc}
\toprule
Protocol & Model & $E_{\rm task}$ & $E_{\rm end}$ & $E_{\rm gen}$ &
$g_{\rm trans}$ & $g_\psi^{\rm pre}$ & $g_\psi^{\rm final}$ &
Switch / law\\
\midrule
Random & $P_{\rm diag}$ finite & $3.27{\pm}0.82$ & $1.09{\pm}0.07$ &
$0.264{\pm}0.128$ & 0.997 & 1.000 & 0.015 & 100\% / 88\%\\
Random & TP-GMM SE(2) & $10.48{\pm}0.56$ & $1.22{\pm}0.37$ &
$0.363{\pm}0.068$ & 0.920 & 0.358 & 0.356 & 0\% / 0\%\\
Random & Full operator & $9.50{\pm}1.17$ & $6.27{\pm}0.16$ &
$0.670{\pm}0.194$ & 0.649 & 0.713 & 0.001 & 70\% / 0\%\\
Random & Generic RBF & $24.12{\pm}0.96$ & $23.28{\pm}0.04$ &
$0.907{\pm}0.091$ & 0.817 & 0.380 & 0.000 & 20\% / 10\%\\
\midrule
Qualified & $P_{\rm diag}$ finite & $3.33{\pm}0.72$ & $1.33{\pm}0.09$ &
$0.264{\pm}0.159$ & 0.981 & 0.999 & 0.020 & 100\% / 92\%\\
Qualified & TP-GMM SE(2) & $10.09{\pm}0.65$ & $0.67{\pm}0.20$ &
$0.354{\pm}0.074$ & 0.961 & 0.356 & 0.360 & 0\% / 0\%\\
Qualified & Full operator & $9.14{\pm}1.20$ & $6.37{\pm}0.05$ &
$0.568{\pm}0.155$ & 0.638 & 0.726 & 0.000 & 80\% / 0\%\\
Qualified & Generic RBF & $28.47{\pm}1.48$ & $28.04{\pm}1.01$ &
$0.963{\pm}0.066$ & 0.382 & 0.444 & 0.000 & 30\% / 0\%\\
\bottomrule
\end{tabular}
\end{table*}
...
\subsection{Observable Progress Recovers the Same Law}
...
\subsection{The Learned Relation Is Invariant to World Orientation}
...
The circular symmetry intervention closes the main intervention-based evidence
chain. Step 0 first
...
The placebo arm makes the alternative hypothesis explicit. A learner that uses
marginal trajectory statistics can observe that yaw motion is present and assign
$\alpha_\psi>0$. A learner that identifies the intervention-response relation
should instead test whether the trajectory changes with $\Delta\psi$. Because
the placebo yaw sweep is fixed across axial-yaw interventions, the
task-consistent prediction is $\alpha_\psi\approx0$.

\begin{figure*}[t]
    \centering
    \includegraphics[width=\textwidth]{symmetry_transfer_main.png}
    \caption{Symmetry-transfer intervention benchmark. A, The keyed, circular-honest,
...
$N{=}3$ is reported for TP-GMM SE(2) only.}
\label{tab:sym_gap}
\small
\setlength{\tabcolsep}{4.0pt}
\begin{tabular}{lcccc}
...
At the sample-starved $N{=}3$ (reported for TP-GMM SE(2) only), the gap drops to
0.414 from 0.680 at $N{=}5$, matching its few-shot disadvantage in
Table~\ref{tab:fewshot_tpgmm}.
...
useful when flexible models also identify the relation
(Fig.~\ref{fig:sym_var}). On the hardest few-shot circular-honest case
($N=5$), the full operator has mean
$E_\alpha=0.062$ with total standard deviation 0.253, seed standard deviation
0.087, subset standard deviation 0.138, and one catastrophic hallucination out
of 18 seed/subset cells ($E_\alpha=1.106$). The finite $P_{\rm diag}$ has mean
$E_\alpha=0.0077$, total standard deviation 0.011, seed standard deviation
0.006, subset standard deviation 0.009, worst case 0.045, and zero
hallucinations. Thus the total standard deviation is about 22 times lower and
the worst case is about 25 times lower under the generator-regularized model.
This contrast is cleanest on the circular-honest arm; on the placebo arm both
models are stable because the fixed yaw sweep is absorbed into the nominal
trajectory. The correct claim is therefore not exclusive accuracy, but
few-shot reliability: an unconstrained $3\times3$ operator has a fat
overfitting tail, whereas the diagonal generator prior does not.

\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{symmetry_variance.png}
    \caption{Few-shot variance of generator-law error on the circular-honest
    arm. The full operator has a high-variance tail at $N=5$, whereas the
    diagonal finite-action prior keeps the law stable across seed/subset cells.}
    \label{fig:sym_var}
\end{figure}

\begin{table}[t]
\centering
\caption{Few-shot reliability on the circular-honest arm at $N=5$ over 18
seed/subset cells. Hallucination denotes fits whose circular yaw relevance
...
\begin{tabular}{lcc}
\toprule
Metric & Full op. & $P_{\rm diag}$ finite\\
\midrule
Mean $E_\alpha$ & 0.062 & 0.0077\\
...
The current intervention study isolates a three-generator subgroup $(x,y,\psi)$
...
excitation checks in all claimed directions, including wrist orientation,
coupled translation-rotation generators, and contact-rich whole-body settings.
The phase variable is also treated as observable progress or a provided
semantic segmentation, not as a learned semantic phase model. A deployable
system would need to estimate this progress online and report errors when the
phase estimate is wrong. The controlled ManiSkill/PhysX experiments are
designed to isolate the generator law under fixed contexts, controllers, and
seeds; the corresponding hardware study should preserve the same three-arm
intervention logic and report the same generator-law metrics rather than
replacing the claim with a raw success-rate comparison.
