...
"""Collect SE(3) 6-generator rollouts (supplement to the SE(2) rotated collector).

Two arms, at the frozen SE(3) anchor (-0.15, 0.00, 0.08) and orientation Q1 (identity):
  - keyed          : KeyedCircularPhaseSwitchSE3-v1, full SE(3) solve_se3.
  - circular_honest: CircularPhaseSwitchSE3-v1, solve_se3 with align_yaw=0 (yaw is
                     a pure SO(2) gauge DOF, so the peg inserts at nominal yaw).
...
# Firm gripper (unchanged from rotated-axis / symmetry-transfer).
...
def collect_se3(rows, output_path, seed, orientation, retries_per_condition,
                task_anchor, yaw_mode, context_manifest_path=None,
                robot_init_qpos_noise=0.0):
    env_id = "KeyedCircularPhaseSwitchSE3-v1" if yaw_mode == "keyed" else "CircularPhaseSwitchSE3-v1"
    task = {
        "keyed": "keyed_to_circular_phase_switch_se3",
        "honest": "circular_phase_switch_se3_honest",
    }[yaw_mode]
    base = gym.make(
        env_id,
        num_envs=1,
    ...
    with h5py.File(output_path, "w") as data_file:
        ...
        data_file.attrs["source_type"] = "motionplanning_physx_phase_switch_se3"
...
def main():
    ...
    parser.add_argument(
        "--yaw-mode",
        choices=["keyed", "honest"],
        default="keyed",
        help="keyed=full SE(3) keyed task; honest=circular flat-yaw gauge.",
    )
