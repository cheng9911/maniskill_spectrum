...
"""Freeze rank-6 few-shot subsets for the LIBERO relation suite."""
...
def diagnostics(contexts, indices):
    ...
    return {
        ...
        "augmented_intercept_rank": int(
            np.linalg.matrix_rank(np.column_stack([np.ones(len(selected)), selected]))
        ),
...
def main():
    ...
    if len(contexts) != 60:
        raise RuntimeError(f"expected 60 mixed contexts, found {len(contexts)}")
    ...
    output = {
        "schema_version": 1,
        "status": "frozen_before_libero_relation_suite_fitting",
        ...
        "policies": {
            "same_subset_indices_across_tasks_and_seeds": True,
            "fit_failures_are_retained": True,
        },
        "subsets": subsets,
    }
    output_path = args.experiment.parent / "libero_relation_suite_subsets.json"
