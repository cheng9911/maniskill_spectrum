...
"""Freeze rank-6 SE(3) mixed-context subsets (se3_subsets.json).

Mirror of prepare_symmetry_transfer_subsets.py generalized to the 6-generator
basis. The mixed pool is 60 six-vectors; qualified subsets must have rank 6 and
condition number < threshold. Sample sizes are {8, 15, 30} (N=5 is impossible
for a rank-6 design).
"""
...
def main():
    ...
    if len(contexts) != 60:
        raise RuntimeError(
            f"SE(3) manifest requires 60 fixed mixed contexts, found {len(contexts)}"
        )
    ...
    output = {
        "schema_version": 2,
        "status": "frozen_before_se3_transfer_fitting",
    ...
    output_path = args.experiment.parent / "se3_subsets.json"
