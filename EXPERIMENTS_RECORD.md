...
> 其中 LIBERO 数据（#18/#19）是**合成探针**（见 §7）；geometry transfer（#20/#21）的迁移预测
> 改用**有限 SE(3) 实现**，但其源/目标剖面仍来自**线性合成探针**（见 §2.4、§7）。
...
|---|---:|---:|---:|
| **Ours transfer**（源 Pdiag N=30 + 目标 nominal N=1） | **1.0** | **2.13e-4** | ~1.8e-7 |
| Pdiag finite target scratch | 1.0 | 3.76e-3 / 9.19e-4 / 5.87e-4 | ~3.5/2.3/2.1e-7 |
| Frame-weighted target scratch | 0.0 | 0.138 / 0.130 / 0.127 | ~8.1/6.6/6.0e-6 |
| Phase scalar GP target scratch | 0.0 | 0.138 / 0.130 / 0.127 | ~8.1/6.6/6.0e-6 |
...
- by_pair：5 个迁移对全部 acc 1.0；e_alpha 分对：drawer→plate 1.35e-5、
  knob→microwave 1.38e-5、bowl_stove→bowl_plate / bowl_stove→cream_cheese /
...
- 结果（N=8，method → m_transfer_acc / e_alpha_mean / e_task_mean）：
  - Ours transfer：**1.0** / 2.13e-4 / 1.86e-7
...
- 解读：TP-GMM SE(3) 目标 scratch 虽能到 acc 1.0，但 e_alpha 高两个数量级（2.6e-2 vs 2.1e-4）、
  e_task 差约 2 个数量级——再次印证"冻结源剖面 + 重估目标名义"的样本效率优势。
...
1. **geometry transfer（#20/#21）迁移预测用有限 SE(3) 实现，但合成数据仍是线性**：
   `benchmark_geometry_transfer.py::_transfer_predict`（约 L145–171）用
   `X = C0 Exp(α(s) ⊙ Log T(c)) C0⁻¹ X0` 的有限形式（与 §2.4、模型 `_local_actions` 一致），
   因此只有"Ours transfer"的 e_task（任务空间 SE(3) metric MSE）变了（9.28e-9 → 1.8e-7）；
   源/目标剖面仍来自**线性合成探针**（`object_pose_at = α·(nominal + selector·δ)`），故 e_alpha
   （2.13e-4）与 target-scratch 基线（本就走有限 `Model.predict`）都不变。旧的线性实现
   （`nominal_curve + context * profile`）及对应 9.28e-9 数字已废弃；迁移指标统一改用 e_task。
