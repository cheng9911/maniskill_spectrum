...
clearance. Fixed-context replications, few-shot tests, rotation-of-the-world
controls, and a preregistered circular symmetry intervention support assigning
geometric task relevance to generators rather than frames.
\end{abstract}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{experiment_overview_scenes.png}
...
Table~\ref{tab:sym_m1} reports M1 at $N=20$. On both circular arms, every
...
failure is structural for scalar frame relevance; and the five-seed/few-shot
replications show that the diagonal finite-action model identifies the law from
few mixed demonstrations.
...
excitation checks in all claimed directions. The work is also simulation-only,
which is useful for causal interventions but does not establish robustness to
perception error, calibration drift, friction uncertainty, or human
demonstration noise.
